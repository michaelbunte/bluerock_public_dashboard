 ...
 * 
 * Modified by Christian Aguilar and Ruben Rodriguez to work with React.
 */
...
function loadLiquidFillGauge(element, value, xpos, ypos, config) {
...
//    const radius = config.radius;//Math.min(parseInt(gauge.style("width")), parseInt(gauge.style("height"))) / 2;

    const locationX= xpos; //parseInt(gauge.style("width")) / 2 - radius;
    const locationY= ypos;// parseInt(gauge.style("height")) / 2 - radius;
    //console.log("attempting to create gauge");
    ...
    const waveRiseScale = d3.scaleLinear()
       ...
       // circle at 100%.
       .range([(height + waveHeight), (fillCircleMargin - waveHeight)])
       .domain([0, 1]);
    ...
    const textRiseScaleY = d3.scaleLinear()
       ...
       .domain([0, 1]);
    //console.log("Current loc " + locationX + ", " + locationY);
    ...
    const clipArea = d3.area()
        ...
        .y1(function (d) { return (width + waveHeight); });
    ...
    const fillCircleGroup = gaugeGroup.append("g")
        ...
        .attr("width", (width)-fillCircleMargin*2)
           .attr("height", height-fillCircleMargin)
        //    .attr("y",0)
        //    .attr("x", 3)
           .style("fill", config.waveColor)
            .attr('transform', 'translate('+ fillCircleMargin + ','+0+')');
        //    .attr("width", fillCircleRadius);
    ...
    function GaugeUpdater() {
        ...
        .tween("text", function () {
            ...
            var waveHeight = width * waveHeightScale(fillPercent * 100);
            var waveRiseScale = d3.scaleLinear()
                // The clipping area size is the height of the fill circle + the wave height, so we position the clip wave
                // such that the it will overlap the fill circle at all when at 0%, and will totally cover the fill
                // circle at 100%.
                .range([(height), (fillCircleMargin - waveHeight)])
                .domain([0, 1]);
            ...
            if (config.waveHeightScaling) {
                newClipArea = d3.area()
                    .x(function (d) { return waveScaleX(d.x); })
                    .y0(function (d) { return waveScaleY(Math.sin(Math.PI * 2 * config.waveOffset * -1 + Math.PI * 2 * (1 - config.waveCount) + d.y * 2 * Math.PI)); })
                    .y1(function (d) { return (height + waveHeight); });
            } else {
...
function LiquidFillGauge(props) {
    ...
    React.useEffect(() => {
        loadLiquidFillGauge(svgRef.current, props.fillLevel, props.xPos,props.yPos, liquidFillGaugeDefaultSettings());
    });
    return (
        <g transform={"scale("+scale+")"} ref={svgRef}></g>
    );
}
